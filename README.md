# <img src="assets/agar-title.svg" width="100%"/>

> bevare agar is under heavy development. all is subject to change

# What is AgarVM?

Agar is a virtual machine. It comes with an assembly language.

You can test agar by writting assembly and running

``` sh
agar assemble [input-file].algae run
```

To view what the assembly looks like, checkout the [tests directory](tests) or read the [documentation directory](docs)
